...
#include "lowbattery-routing-protocol.h"
#include "ns3/log.h"
...
NS_LOG_COMPONENT_DEFINE ("LowbatteryRoutingProtocol");

namespace lowbattery {
NS_OBJECT_ENSURE_REGISTERED (RoutingProtocol);

/// UDP Port for LOWBATTERY control traffic
const uint32_t RoutingProtocol::LOWBATTERY_PORT = 654;
...
{
  static TypeId tid =
      TypeId ("ns3::lowbattery::RoutingProtocol")
          .SetParent<Ipv4RoutingProtocol> ()
          .SetGroupName ("Lowbattery")
...
RoutingProtocol::PrintRoutingTable (Ptr<OutputStreamWrapper> stream, Time::Unit unit) const
{
  *stream->GetStream () << "Node: " << m_ipv4->GetObject<Node> ()->GetId ()
                        ...
                        << ", LOWBATTERY Routing table" << std::endl;
...
RoutingProtocol::NotifyInterfaceUp (uint32_t i)
{
  ...
  NS_ASSERT (socket != 0);
  socket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvLowbattery, this));
  socket->BindToNetDevice (l3->GetNetDevice (i));
  socket->Bind (InetSocketAddress (iface.GetLocal (), LOWBATTERY_PORT));
  ...
  socket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvLowbattery, this));
  socket->BindToNetDevice (l3->GetNetDevice (i));
  socket->Bind (InetSocketAddress (iface.GetBroadcast (), LOWBATTERY_PORT));
  socket->SetAllowBroadcast (true);
  socket->SetIpRecvTtl (true);
  ...
  if (id == 1)
    {
      ...
      //std::cout << "a + " << "\n";
      //Simulator::Schedule (Seconds (20.0), &RoutingProtocol::SendLowbatteryBroadcast, this, id, 9, 100, 600, 0);
      //SendLowbatteryBroadcast(id, 9,100,600, 0);
      //sourcecount[id]=1;
      //source_time[id]=time;
    }
      ...
      if (id == source_list[time - Grobal_StartTime])
        {
          ...
          Simulator::Schedule (Seconds (shift_time), &RoutingProtocol::SendLowbatteryBroadcast, this, source_list[index_time],
...
void
RoutingProtocol::SendToLowbattery (Ptr<Socket> socket, Ptr<Packet> packet, Ipv4Address destination,
                                  int32_t hopcount, int32_t des_id)
{
      broadcastCount++;
      socket->SendTo (packet, 0, InetSocketAddress (destination, LOWBATTERY_PORT));
      //int time = Simulator::Now ().GetSeconds ();
...
void
RoutingProtocol::SendLowbatteryBroadcast (int32_t pri_value, int32_t des_id, int32_t des_x,
                                         int32_t des_y, int32_t hopcount) //pri_value 自分のid
{
  ...
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j = m_socketAddresses.begin ();
       j != m_socketAddresses.end (); ++j)
    {
      ...
      TypeHeader tHeader (LOWBATTERYTYPE_SEND);
      ...
      if (iface.GetMask () == Ipv4Mask::GetOnes ())
        ...
        Simulator::Schedule (Jitter, &RoutingProtocol::SendToLowbattery, this,
...
void
RoutingProtocol::RecvLowbattery (Ptr<Socket> socket)
{
  int32_t id = m_ipv4->GetObject<Node> ()->GetId ();
  ...
  TypeHeader tHeader (LOWBATTERYTYPE_HELLO);
  ...
  switch (tHeader.Get ())
    {
      case LOWBATTERYTYPE_HELLO: { //hello message を受け取った場合
      ...
      case LOWBATTERYTYPE_SEND: {
        ...
        if (des_id == id || myDistance < 1000) //宛先が自分だったら
          ...
          break;
            ...
            {
              // SendLowbatteryBroadcast (int32_t pri_value, int32_t des_id, int32_t des_x,
              ...
              SendLowbatteryBroadcast(source_id, des_id, des_x, des_y, hopcount);
