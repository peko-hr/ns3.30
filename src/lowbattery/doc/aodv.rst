...
Ad Hoc On-Demand Distance Vector (LOWBATTERY)
---------------------------------------

This model implements the base specification of the Ad Hoc On-Demand 
Distance Vector (LOWBATTERY) protocol. The implementation is based on 
:rfc:`3561`.

The model was written by Elena Buchatskaia and Pavel Boyko of ITTP RAS,
and is based on the ns-2 LOWBATTERY model developed by the CMU/MONARCH group
and optimized and tuned by Samir Das and Mahesh Marina, University of
Cincinnati, and also on the LOWBATTERY-UU implementation by Erik Nordström of 
Uppsala University.

Model Description
*****************

The source code for the LOWBATTERY model lives in the directory `src/lowbattery`.

Design
++++++

Class ``ns3::lowbattery::RoutingProtocol`` implements all functionality of 
...
forwarding.  The first one, ``ns3::lowbattery::RouteOutput``, is used for 
locally originated packets, and the second one, ``ns3::lowbattery::RouteInput``,
...
this functionality are attributes of ``ns3::lowbattery::RoutingProtocol``. 
...
LOWBATTERY discovers routes on demand.  Therefore, the LOWBATTERY model buffers all 
packets while a route request packet (RREQ) is disseminated. 
A packet queue is implemented in lowbattery-rqueue.cc. A smart pointer to 
...
* This LOWBATTERY implementation can detect the presence of unidirectional 
  ...
  This heuristic is taken from LOWBATTERY-UU implementation and can be disabled.
...
the assertion from the LOWBATTERY RFC that LOWBATTERY works over UDP.  This model uses
