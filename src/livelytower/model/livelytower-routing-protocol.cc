...
#include "livelytower-routing-protocol.h"
#include "ns3/log.h"
...
NS_LOG_COMPONENT_DEFINE ("LivelytowerRoutingProtocol");

namespace livelytower {
...
{
  static TypeId tid =
      TypeId ("ns3::livelytower::RoutingProtocol")
          .SetParent<Ipv4RoutingProtocol> ()
          .SetGroupName ("Livelytower")
...
RoutingProtocol::NotifyInterfaceUp (uint32_t i)
{
  ...
  NS_ASSERT (socket != 0);
  socket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvLivelytower, this));
...
void
RoutingProtocol::RecvLivelytower (Ptr<Socket> socket)
{
  int32_t id = m_ipv4->GetObject<Node> ()->GetId ();
  std::cout << "In recv Livelytower(Node " << m_ipv4->GetObject<Node> ()->GetId () << ")\n";
  Ptr<MobilityModel> mobility = m_ipv4->GetObject<Node> ()->GetObject<MobilityModel> ();
