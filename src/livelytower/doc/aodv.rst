...
The source code for the SAMPLE model lives in the directory `src/livelytower`.

Design
++++++

Class ``ns3::livelytower::RoutingProtocol`` implements all functionality of 
...
forwarding.  The first one, ``ns3::livelytower::RouteOutput``, is used for 
locally originated packets, and the second one, ``ns3::livelytower::RouteInput``,
...
this functionality are attributes of ``ns3::livelytower::RoutingProtocol``. 
...
A packet queue is implemented in livelytower-rqueue.cc. A smart pointer to 
