...
Ad Hoc On-Demand Distance Vector (HIRAI)
---------------------------------------

This model implements the base specification of the Ad Hoc On-Demand 
Distance Vector (HIRAI) protocol. The implementation is based on 
:rfc:`3561`.

The model was written by Elena Buchatskaia and Pavel Boyko of ITTP RAS,
and is based on the ns-2 HIRAI model developed by the CMU/MONARCH group
and optimized and tuned by Samir Das and Mahesh Marina, University of
Cincinnati, and also on the HIRAI-UU implementation by Erik Nordström of 
Uppsala University.

Model Description
*****************

The source code for the HIRAI model lives in the directory `src/hirai`.

Design
++++++

Class ``ns3::hirai::RoutingProtocol`` implements all functionality of 
...
forwarding.  The first one, ``ns3::hirai::RouteOutput``, is used for 
locally originated packets, and the second one, ``ns3::hirai::RouteInput``,
...
this functionality are attributes of ``ns3::hirai::RoutingProtocol``. 
...
HIRAI discovers routes on demand.  Therefore, the HIRAI model buffers all 
packets while a route request packet (RREQ) is disseminated. 
A packet queue is implemented in hirai-rqueue.cc. A smart pointer to 
...
* This HIRAI implementation can detect the presence of unidirectional 
  ...
  This heuristic is taken from HIRAI-UU implementation and can be disabled.
...
the assertion from the HIRAI RFC that HIRAI works over UDP.  This model uses
