...
#define WindowSize 10000000 //NEARESTのウィンドウサイズ microsecond   = 10second
...
private:
  ...
  void SendToNearest (Ptr<Socket> socket, Ptr<Packet> packet, Ipv4Address destination,
                        int32_t hopcount, int32_t des_id); //Nearest broadcast の送信
  ...
  void
  SendNearestBroadcast (int32_t pri_value, int32_t des_id, int32_t des_x, int32_t des_y,
