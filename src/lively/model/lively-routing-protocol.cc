...
#include "lively-routing-protocol.h"
#include "ns3/log.h"
...
NS_LOG_COMPONENT_DEFINE ("LivelyRoutingProtocol");

namespace lively {
NS_OBJECT_ENSURE_REGISTERED (RoutingProtocol);

/// UDP Port for LIVELY control traffic
const uint32_t RoutingProtocol::LIVELY_PORT = 654;
...
{
  static TypeId tid =
      TypeId ("ns3::lively::RoutingProtocol")
...
RoutingProtocol::NotifyInterfaceUp (uint32_t i)
{
  ...
  NS_ASSERT (socket != 0);
  socket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvLively, this));
  socket->BindToNetDevice (l3->GetNetDevice (i));
  socket->Bind (InetSocketAddress (iface.GetLocal (), LIVELY_PORT));
  ...
  socket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvLively, this));
  socket->BindToNetDevice (l3->GetNetDevice (i));
  socket->Bind (InetSocketAddress (iface.GetBroadcast (), LIVELY_PORT));
  socket->SetAllowBroadcast (true);
  socket->SetIpRecvTtl (true);
  ...
  if (id == 1)
    {
      ...
      //std::cout << "a + " << "\n";
      //Simulator::Schedule (Seconds (20.0), &RoutingProtocol::SendLivelyBroadcast, this, id, 9, 100, 600, 0);
      //SendLivelyBroadcast(id, 9,100,600, 0);
      //sourcecount[id]=1;
      //source_time[id]=time;
    }
      ...
      if (id == source_list[time - Grobal_StartTime])
        {
          ...
          Simulator::Schedule (Seconds (shift_time), &RoutingProtocol::SendLivelyBroadcast, this, source_list[index_time],
...
{
      broadcastCount++;
      socket->SendTo (packet, 0, InetSocketAddress (destination, LIVELY_PORT));
      //int time = Simulator::Now ().GetSeconds ();
...
void
RoutingProtocol::SendLivelyBroadcast (int32_t pri_value, int32_t des_id, int32_t des_x,
                                         int32_t des_y, int32_t hopcount) //pri_value 自分のid
{
  ...
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j = m_socketAddresses.begin ();
       j != m_socketAddresses.end (); ++j)
    {
      ...
      TypeHeader tHeader (LIVELYTYPE_SEND);
      ...
      if (iface.GetMask () == Ipv4Mask::GetOnes ())
        ...
        Simulator::Schedule (Jitter, &RoutingProtocol::SendToLively, this,
...
void
RoutingProtocol::RecvLively (Ptr<Socket> socket)
{
  int32_t id = m_ipv4->GetObject<Node> ()->GetId ();
  ...
  TypeHeader tHeader (LIVELYTYPE_HELLO);
  ...
  switch (tHeader.Get ())
    {
      case LIVELYTYPE_HELLO: { //hello message を受け取った場合
        ...
        if (des_id == id || myDistance < 1000) //宛先が自分だったら
          ...
          break;
            ...
            if(sourceDistance > myDistance)
            {
              ...
              SendLivelyBroadcast(source_id, des_id, des_x, des_y, hopcount);
