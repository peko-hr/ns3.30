...
Ad Hoc On-Demand Distance Vector (LIVELY)
---------------------------------------

This model implements the base specification of the Ad Hoc On-Demand 
Distance Vector (LIVELY) protocol. The implementation is based on 
:rfc:`3561`.

The model was written by Elena Buchatskaia and Pavel Boyko of ITTP RAS,
and is based on the ns-2 LIVELY model developed by the CMU/MONARCH group
and optimized and tuned by Samir Das and Mahesh Marina, University of
Cincinnati, and also on the LIVELY-UU implementation by Erik Nordström of 
Uppsala University.

Model Description
*****************

The source code for the LIVELY model lives in the directory `src/lively`.

Design
++++++

Class ``ns3::lively::RoutingProtocol`` implements all functionality of 
...
forwarding.  The first one, ``ns3::lively::RouteOutput``, is used for 
locally originated packets, and the second one, ``ns3::lively::RouteInput``,
...
this functionality are attributes of ``ns3::lively::RoutingProtocol``. 
...
LIVELY discovers routes on demand.  Therefore, the LIVELY model buffers all 
packets while a route request packet (RREQ) is disseminated. 
A packet queue is implemented in lively-rqueue.cc. A smart pointer to 
...
* This LIVELY implementation can detect the presence of unidirectional 
  ...
  This heuristic is taken from LIVELY-UU implementation and can be disabled.
...
the assertion from the LIVELY RFC that LIVELY works over UDP.  This model uses
