  ...
  if (m_lossModel == 3)
    {
      ...
      wifiChannel.AddPropagationLoss (m_lossModelName, "MaxRange",
                                        DoubleValue (400));//伝搬距離
...
VanetRoutingExperiment::SetupScenario ()
{
  ...
  if (m_scenario == 1)
  ...
  else if (m_scenario == 3)
    {
      ...
      //m_traceFile = "src/wave/examples/no_signal/no_signal_" + std::to_string(node_num) + ".tcl"; //モビリティ入力ファイル
      //m_traceFile = "src/wave/examples/no_signal/no_signal_200.tcl"; //モビリティ入力ファイル
      m_traceFile = "src/hirai/ped2000pas1000.tcl"; //モビリティ入力ファイル
